#/
# @param as		Chunkposition maker (tag=pcd_chunkpos)
#/


execute at @e[tag=pcd_displaypos,limit=1] run function zzz_chunkdisplay:loop_detect_chunk/body

execute as @e[tag=pcd_displaypos,limit=1] at @s run tp ~1 ~ ~
execute at @s run tp ~16 ~ ~
scoreboard players operation X pcd_tmp += 1 pcd_const


execute if score X pcd_tmp >= SIZE_X pcd_tmp as @e[tag=pcd_displaypos,limit=1] at @s run tp ~ ~ ~1
execute if score X pcd_tmp >= SIZE_X pcd_tmp at @s run tp ~ ~ ~16

execute if score X pcd_tmp >= SIZE_X pcd_tmp as @e[tag=pcd_displaypos,limit=1] store result entity @s Pos[0] double 0.5 run scoreboard players get DISPLAY_X pcd_tmp
